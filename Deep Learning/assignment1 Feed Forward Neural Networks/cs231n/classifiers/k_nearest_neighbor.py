...
class KNearestNeighbor:
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i,test in enumerate(X):
      ...
      for j,train in enumerate(self.X_train):
        ...
        #dists[i,j]=np.sum(np.power((train-test),2))
        #####################################################################
        # TODO:                                                             #
        ...
        #print train.shape
        #print test.shape
        dists[i,j]=np.sqrt(np.sum(np.power((test-train),2)))
        #print dists[i,j]
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in xrange(num_test):
      ...
      #print (test-self.X_train).shape
      #return
      temp=test-self.X_train
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    x_test_squared=np.power(X,2)
    x_train_squared=np.power(self.X_train,2)
    ...
    dists=np.sqrt((np.sum((x[:,np.newaxis]+y),axis=2))-2*np.dot(X,self.X_train.T))
    
    #dists=(np.sum((x_test_squared[:,np.newaxis]+x_train_squared),axis=2))-2*np.dot(X,self.X_train.T)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      relevant_distance=dists[i,:]
      #relevant_distance_sorted=np.sort(relevant_distance,axis=None)
      
      relevant_index=np.argsort(relevant_distance)[:k]
      
      closest_y=self.y_train[relevant_index]
