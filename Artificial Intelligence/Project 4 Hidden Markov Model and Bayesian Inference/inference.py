...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #the allPossible is dictionary the key of the dictionary is the possible ghost location
        #the value of the dictionary is the probab. for that state location



        if noisyDistance==None:
            #this is the case where the ghost has been eaten
            #for ghost eaten all probab. of the ghost is in the jail location
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    ...
                    #here i am performing the update for P(X|E)=P(E|X)*P(E)
                    # X = true probab distribution of state 
                    # E = observed evidence 
                    # P(E|X) is the emission model ie likelihood
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        elapseTime_belief=util.Counter()
        #self.legalPosition stores all the allowed leagal position that the ghost can reside in.
        for ghost_position in self.legalPositions:
            present_ghost_position_belief=self.beliefs[ghost_position]
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, ghost_position))

            for position,position_prob in newPosDist.iteritems():
                elapseTime_belief[position]+=present_ghost_position_belief*newPosDist[position]

        elapseTime_belief.normalize()
        self.beliefs=elapseTime_belief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # board_position=self.legalPositions
        # num_particles=self.numParticles
        # lst=[]
        # for _ in range(num_particles):
        #     lst.append(choice(board_position))
        # self.particles=lst
        n = self.numParticles
        legal = self.legalPositions
        uniformInit = []
        #this function keeps adding particles into the list until we've eclipsed the number of particles we want.
        while n > 0:
            # create a particle for each position, reduce n
            if n > len(legal):
                uniformInit += legal
                n -= len(legal)
            else:
                uniformInit += legal[0:n-1]
                n = 0

        self.particles = uniformInit
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance==None:
            ...
            temp_counter[self.getJailPosition]=1
            self.beliefs=temp_counter


        else:
            temp_counter=util.Counter()
            for particle_position in self.particles:
                trueDistance=util.manhattanDistance(particle_position,pacmanPosition)
                temp_counter[particle_position]+=emissionModel[trueDistance]
            self.beliefs=temp_counter
        ...
        if self.beliefs.totalCount() == 0:
            ...
            for i in range(len(self.particles)):
                #this allows me to sample based on the particle weights
    ...
    def getBeliefDistribution(self):
        ...
        for position in self.particles:
            # print position
            belief_distribution[position]+=1
